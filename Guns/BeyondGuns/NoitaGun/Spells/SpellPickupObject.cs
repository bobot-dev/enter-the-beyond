...
{ 
    ...
    class SpellPickupObject : PickupObject, IPlayerInteractable
    {
		...
		public override void Pickup(PlayerController player)
        {
			...
			{
				BotsModule.Log($"Warning spellToGive on \"{this.name}\" is null");
			}
			else
			{
				if (this.spellToGive.addsComponents)
				{
		...
		public void InitSpell(SpellTypes type, Spell spell)
		{
			StaticSpellReferences.validSpells.Add(type, spell);
			StaticSpellReferences.JUSTFUCKINGWORK.Add(type.ToString(), base.PickupObjectId);
			
			StaticSpellReferences.spellLootTable.AddItemToPool(base.PickupObjectId);
		...
		private void Start()
		{
			...
			if (StaticSpellReferences.validSpells[type] != null)
            {
				this.spellToGive = StaticSpellReferences.validSpells[type];
			}
