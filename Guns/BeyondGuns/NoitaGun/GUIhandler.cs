...
namespace LiveRecolor
{
    ...
    class GUIhandler : MonoBehaviour
    {
        ...
        void OnColorPressed()
        {
            if (BotsModule.debugMode) BotsModule.Log("thing press yes");

        }
        //basically updates our color labels to the body part thats picked
        void OnBodyPickerIndexChanged()
        {
            if (BotsModule.debugMode) BotsModule.Log("thing press yes yes");
            if (m_player.CurrentGun.gameObject.GetComponent<Wand>() != null)
            {
                if (BotsModule.debugMode) BotsModule.Log("not null woooo");
                ...
                if (BotsModule.debugMode) ETGModConsole.Log((Wand.spells[bodyPartPicker.GetIndex()].index).ToString());
