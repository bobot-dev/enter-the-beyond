	...
	public static class StaticReferences
	{
		...
		public static void Init()
		{
			...
			foreach (string text in StaticReferences.assetBundleNames)
			{
				...
				{
					DebugUtility.PrintError<string>("Failed to load asset bundle: " + text, "FF0000");
			...
			foreach (KeyValuePair<string, string> keyValuePair in StaticReferences.roomTableMap)
			{
				...
				{
					DebugUtility.PrintError<string>("Failed to load room table: " + keyValuePair.Key + ":" + keyValuePair.Value, "FF0000");
					DebugUtility.PrintException(e2, "FF0000");
				}
			}
			DebugUtility.Print<string>("Static references initialized.", "FFFFFF", false);
		}
