	...
	public static class RoomFactory
	{
        ...
        public static PrototypeDungeonRoom Build(Texture2D texture, RoomData roomData, bool SetRoomCategory, bool AutoAssignToFloor, bool AssignDecorationProperties, float? Weight = 1)
        {
            try
            {
                ...
                //DebugUtility.PrintError($"{room.name}: isDarkRoom = {roomData.isDarkRoom}");
            ...
            {
                DebugUtility.PrintError("Failed to build room!");
        ...
        {
            // DebugUtility.Print("Building Exits...");
            ...
            // DebugUtility.Print("Adding Objects...");
            ...
            if (autoAssignToFloor && roomData.floors != null)
            {
                ...
                if (room.category == PrototypeDungeonRoom.RoomCategory.SECRET)
                {
                    ...
                    BeyondPrefabs.SecretRoomTable.includedRooms.elements.Add(ModRoomPrefabs.GenerateWeightedRoom(room, Weight.Value));
                }
                else
                {
        ...
        public static void AssignRoomToFloorRoomTable(PrototypeDungeonRoom Room, GlobalDungeonData.ValidTilesets targetTileSet, float? Weight)
        {
            ...
            {
                BeyondPrefabs.CastleRoomTable.includedRooms.elements.Add(ModRoomPrefabs.GenerateWeightedRoom(Room, Weight.Value));
            }
            else if (targetTileSet == GlobalDungeonData.ValidTilesets.SEWERGEON)
            {
                BeyondPrefabs.SewersRoomTable.includedRooms.elements.Add(ModRoomPrefabs.GenerateWeightedRoom(Room, Weight.Value));
            }
            else if (targetTileSet == GlobalDungeonData.ValidTilesets.GUNGEON)
            {
                BeyondPrefabs.Gungeon_RoomTable.includedRooms.elements.Add(ModRoomPrefabs.GenerateWeightedRoom(Room, Weight.Value));
            }
            else if (targetTileSet == GlobalDungeonData.ValidTilesets.CATHEDRALGEON)
            {
                BeyondPrefabs.AbbeyRoomTable.includedRooms.elements.Add(ModRoomPrefabs.GenerateWeightedRoom(Room, Weight.Value));
            }
            else if (targetTileSet == GlobalDungeonData.ValidTilesets.MINEGEON)
            {
                BeyondPrefabs.MinesRoomTable.includedRooms.elements.Add(ModRoomPrefabs.GenerateWeightedRoom(Room, Weight.Value));
            }
            else if (targetTileSet == GlobalDungeonData.ValidTilesets.CATACOMBGEON)
            {
                BeyondPrefabs.CatacombsRoomTable.includedRooms.elements.Add(ModRoomPrefabs.GenerateWeightedRoom(Room, Weight.Value));
            }
            ...
            {
                BeyondPrefabs.ForgeRoomTable.includedRooms.elements.Add(ModRoomPrefabs.GenerateWeightedRoom(Room, Weight.Value));
            }
            else if (targetTileSet == GlobalDungeonData.ValidTilesets.HELLGEON)
            {
                BeyondPrefabs.BulletHellRoomTable.includedRooms.elements.Add(ModRoomPrefabs.GenerateWeightedRoom(Room, Weight.Value));
            }
            return;
        }
        ...
        public static PrototypeDungeonRoom CreateEmptyRoom(int width = 12, int height = 12)
        {
            try
            {
                DebugUtility.Print("  Create Empty Room...", "5599FF");
                PrototypeDungeonRoom room = GetNewPrototypeDungeonRoom(width, height);
        ...
        public static GameObject GetGameObjectFromBundles(string assetPath)
        {
            ...
            {
                return BeyondPrefabs.shared_auto_001.LoadAsset<GameObject>(assetPath);
            }
            else if (BeyondPrefabs.shared_auto_002.LoadAsset<GameObject>(assetPath))
            {
                return BeyondPrefabs.shared_auto_002.LoadAsset<GameObject>(assetPath);
            }
            else if (BeyondPrefabs.braveResources.LoadAsset<GameObject>(assetPath))
            {
                return BeyondPrefabs.braveResources.LoadAsset<GameObject>(assetPath);
            }
            else
            {
                return null;
        ...
        {
            if (BeyondPrefabs.shared_auto_001.LoadAsset<DungeonPlaceable>(assetPath))
            {
                return BeyondPrefabs.shared_auto_001.LoadAsset<DungeonPlaceable>(assetPath);
            }
            else if (BeyondPrefabs.shared_auto_002.LoadAsset<DungeonPlaceable>(assetPath))
            {
                return BeyondPrefabs.shared_auto_002.LoadAsset<DungeonPlaceable>(assetPath);
            }
            else if (BeyondPrefabs.braveResources.LoadAsset<DungeonPlaceable>(assetPath))
            {
                return BeyondPrefabs.braveResources.LoadAsset<DungeonPlaceable>(assetPath);
            }
            else
            {
                return null;
        ...
        public static void AddPlaceableToRoom(PrototypeDungeonRoom room, Vector2 location, string assetPath)
        {
            try
            {
                ...
                {
                    DebugUtility.PrintError("Unable to find asset in asset bundles: " + assetPath, "FF0000");
                }
            }
            catch (Exception e)
            {
