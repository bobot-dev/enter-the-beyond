...
namespace GungeonAPI
{
	...
	public static class DungeonHandler
	{
		...
		{
			DebugUtility.Print<string>("Attempting to override floor layout...", "5599FF", false);
			...
			if (flag)
			{
				...
				DebugUtility.Print<string>("Dungeon name: " + dungeon.name, "FFFFFF", false);
				DebugUtility.Print<string>("Override Flow set to: " + flow.name, "FFFFFF", false);
			}
			dungeon = null;
		}
		...
		public static void CollectDataForAnalysis(DungeonFlow flow, Dungeon dungeon)
		{
			try
			{
				foreach (WeightedRoom weightedRoom in flow.fallbackRoomTable.includedRooms.elements)
				{
					...
					DebugUtility.Print<string>(str + str2, "FFFFFF", false);
				}
			}
			catch (Exception e)
			{
				DebugUtility.PrintException(e, "FF0000");
			}
		}
		...
		public static void LogProtoRoomData(PrototypeDungeonRoom room)
		{
			...
			DebugUtility.Print<string>("==LAYERS==", "FFFFFF", false);
