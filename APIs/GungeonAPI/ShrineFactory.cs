	...
	public class ShrineFactory
	{
		...
		public GameObject Build()
		{
			...
			try
			{
				...
				DebugUtility.Print<string>("Added shrine: " + text, "FFFFFF", false);
				result = gameObject;
			}
			catch (Exception e)
			{
		...
		public static void PlaceBreachShrines()
		{
			...
			foreach (GameObject gameObject in ShrineFactory.registeredShrines.Values)
			{
				try
				{
					...
					if (!flag)
					{
						DebugUtility.Print<string>("    " + gameObject.name, "FFFFFF", false);
		...
		public class CustomShrineController : DungeonPlaceableBehaviour
		{
			...
			private void Start()
			{
				...
				if (!flag2)
				{
					...
					DebugUtility.Print<string>("Started shrine: " + text, "FFFFFF", false);
				}
			}
