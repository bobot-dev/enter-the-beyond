...
namespace CustomCharacters
{
    ...
    public static class Loader
    {
        ...
        private static void LoadDirectories()
        {
            ...
            ToolsGAPI.Print("# of character folders found: " + directories.Length);
            for (int i = 0; i < directories.Length; i++)
            {
                ToolsGAPI.StartTimer("Loading data for " + Path.GetFileName(directories[i]));
                ToolsGAPI.Print("");
                ToolsGAPI.Print("--Loading " + Path.GetFileName(directories[i]) + "--", "0000FF");
                ...
                {
                    ToolsGAPI.PrintError($"No \"{DataFile}\" file found for " + Path.GetFileName(directories[i]));
                    continue;
                }
                ...
                string altSpritesDir = Path.Combine(customCharacterDir, "alt");
