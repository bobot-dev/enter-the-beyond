        ...
        public static void Init()
        {
            ...
            foreach (var name in assetBundleNames)
            {
                try
                {
                    ...
                    if (bundle == null)
                    {
                        ToolsGAPI.PrintError($"Failed to load asset bundle: {name}");
                        continue;
                    }
            ...
            foreach (var entry in roomTableMap)
            {
                ...
                {
                    ToolsGAPI.PrintError($"Failed to load room table: {entry.Key}:{entry.Value}");
                    ToolsGAPI.PrintException(e);
                }
            }
            ...
            foreach (var entry in specialRoomTableMap)
            {
                ...
                {
                    ToolsGAPI.PrintError($"Failed to load special room table: {entry.Key}:{entry.Value}");
                    ToolsGAPI.PrintException(e);
                }
            }
            ...
            //{
            //    ToolsGAPI.LogPropertiesAndFields(data, data.annotation);
            //}

            ToolsGAPI.Print("Static references initialized.");
        }
