...
public class FireLotsOfLasersBehaviour : BasicAttackBehavior
{
	...
	public override BehaviorResult Update()
	{
		...
		this.m_state = FireLotsOfLasersBehaviour.State.PreCharging;
	...
	public override ContinuousBehaviorResult ContinuousUpdate()
	{
		...
		if (this.m_state == FireLotsOfLasersBehaviour.State.PreCharging)
		{
			if (!this.LaserActive)
			{
				this.ChargeFiringLaser(this.chargeTime);
				this.m_timer = this.chargeTime;
				this.m_state = FireLotsOfLasersBehaviour.State.Charging;
			}
		}
		else
		{
			if (this.m_state == FireLotsOfLasersBehaviour.State.Charging)
			{
				...
				if (this.m_timer <= 0f)
				{
					this.m_state = FireLotsOfLasersBehaviour.State.Firing;
			...
			if (this.m_state == FireLotsOfLasersBehaviour.State.Firing)
			{
	...
	protected IEnumerator FireBeam(AIBeamShooter2 aibeamShooter2, int j)
	{
		...
		while (beamCont != null && this.IsfiringLaser)
		{
			...
			clampedAngle = BraveMathCollege.ClampAngle360(this.LaserAngle + (j * 60));
	...
	private FireLotsOfLasersBehaviour.State m_state;
