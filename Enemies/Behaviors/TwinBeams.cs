...
public class TwinBeams : BasicAttackBehavior
{
	...
	public override BehaviorResult Update()
	{
		...
		this.m_state = TwinBeams.State.PreCharging;
		...
		beam1 = m_aiActor.gameObject.GetComponents(typeof(AIBeamShooter2))[0] as AIBeamShooter2;
		beam2 = m_aiActor.gameObject.GetComponents(typeof(AIBeamShooter2))[1] as AIBeamShooter2;
		beam3 = m_aiActor.gameObject.GetComponents(typeof(AIBeamShooter2))[2] as AIBeamShooter2;
		beam4 = m_aiActor.gameObject.GetComponents(typeof(AIBeamShooter2))[3] as AIBeamShooter2;
	...
	public override ContinuousBehaviorResult ContinuousUpdate()
	{
		...
		if (this.m_state == TwinBeams.State.PreCharging)
		{
			if (!this.LaserActive)
			{
				this.ChargeFiringLaser(this.chargeTime);
				this.m_timer = this.chargeTime;
				this.m_state = TwinBeams.State.Charging;
			}
		}
		else
		{
			if (this.m_state == TwinBeams.State.Charging)
			{
				
				this.m_timer -= this.m_deltaTime;
				if (this.m_timer <= 0f)
				{
					this.m_state = TwinBeams.State.Firing;
			...
			if (this.m_state == TwinBeams.State.Firing)
			{
	...
	public void StartFiringTheLaser()
	{
		...
		yes.StartCoroutine(this.FireBeam(beam3, 2));
		yes.StartCoroutine(this.FireBeam(beam4, 3));
	...
	protected IEnumerator FireBeam(AIBeamShooter2 aibeamShooter2, int j)
	{
		...
		while (beamCont != null && this.IsfiringLaser)
		{
			...
			clampedAngle = BraveMathCollege.ClampAngle360((j == 1 || j == 3 ? this.AlaserAngle2 : this.AlaserAngle )) + (90 * j);
	...
	private TwinBeams.State m_state;
	...
	public AIBeamShooter2 beam3;
	public AIBeamShooter2 beam4;
