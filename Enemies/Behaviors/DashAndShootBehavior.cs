...
namespace BotsMod
{
	class DashAndShootBehavior : BasicAttackBehavior
	{
		// Token: 0x060046EA RID: 18154 RVA: 0x0002E882 File Offset: 0x0002CA82
		public DashAndShootBehavior()
		{
			this.dashDirection = DashAndShootBehavior.DashDirection.Random;
		...
		public override BehaviorResult Update()
		{
			...
			{
				SetState(DashAndShootBehavior.DashState.Charge);
			}
			else
			{
				SetState(DashAndShootBehavior.DashState.Dash);
			}
		...
		public override ContinuousBehaviorResult ContinuousUpdate()
		{
			...
			if (this.State == DashAndShootBehavior.DashState.Charge)
			{
			...
			else if (this.State == DashAndShootBehavior.DashState.Dash)
			{
			...
			else if (this.State == DashAndShootBehavior.DashState.Idle)
			{
				return ContinuousBehaviorResult.Finished;
			}
		...
		public override void EndContinuousUpdate()
		{
			...
			if (this.dashCount > 0f && dashCount > m_dashCount)
			{
				this.m_cooldownTimer = 0f;
				this.m_lastDashWasDouble = true;
				BotsModule.Log(dashCount + ", " + m_dashCount);
				m_dashCount++;
			}
			else
			{
				...
				SetState(DashAndShootBehavior.DashState.Idle);
		...
		{
			if (this.m_state == DashAndShootBehavior.DashState.Dash && this.m_shouldFire && clip.GetFrame(frame).eventInfo == "fire")
			{
				this.Fire();
			}
		}
		...
		{
			if (this.m_state == DashAndShootBehavior.DashState.Dash)
			{
		...
		private float[] GetDirections()
		{
			...
			if (this.dashDirection == DashAndShootBehavior.DashDirection.PerpendicularToTarget)
			{
			...
			else if (this.dashDirection == DashAndShootBehavior.DashDirection.KindaTowardTarget)
			{
			...
			else if (this.dashDirection == DashAndShootBehavior.DashDirection.TowardTarget)
			{
			...
			else if (this.dashDirection == DashAndShootBehavior.DashDirection.Random)
			{
		...
		private void Fire()
		{
            ...
            {
				if (this.secondBulletScript != null && !this.secondBulletScript.IsNull)
                {
					this.m_bulletSource.BulletScript = this.secondBulletScript;
				}
				m_scriptCount--;
			}
		...
		private DashAndShootBehavior.DashState State
		{
			get
			{
				return this.m_state;
		...
		private void SetState(DashAndShootBehavior.DashState value)
		{
		...
		private void BeginState(DashAndShootBehavior.DashState state)
		{
			if (state == DashAndShootBehavior.DashState.Charge)
			{
			...
			else if (state == DashAndShootBehavior.DashState.Dash)
			{
				if (this.bulletScript != null && !this.bulletScript.IsNull)
				{
				...
				{
					this.m_aiShooter.ToggleGunAndHandRenderers(false, "DashAndShootBehavior");
				}
				if (this.toggleTrailRenderer && this.m_trailRenderer != null)
				{
					foreach (var dumbfuckingidiot in this.m_trailRenderer)
		...
		private void EndState(DashAndShootBehavior.DashState state)
		{
			if (state == DashAndShootBehavior.DashState.Dash)
			{
				if (!string.IsNullOrEmpty(this.dashAnim))
				{
				...
				if (this.hideGun && this.m_aiShooter)
				{
					this.m_aiShooter.ToggleGunAndHandRenderers(true, "DashAndShootBehavior");
				}
				if (this.toggleTrailRenderer && this.m_trailRenderer != null)
				{
					foreach (var dumbfuckingidiot in this.m_trailRenderer)
		...
		public DashAndShootBehavior.DashDirection dashDirection;
		...
		[InspectorCategory("Attack")]
		public BulletScriptSelector secondBulletScript;
		...
		private DashAndShootBehavior.DashState m_state;
