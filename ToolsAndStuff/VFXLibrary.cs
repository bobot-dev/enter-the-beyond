...
namespace FerryMansOar
{
    public static class VFXLibrary
    {
        ...
        /// <param name="emissivePowers">How strong the glow is</param>
        /// <param name="emissiveColorPowers">not sure what this one dose exactly i just set it to 55 and it worked ¯\_(ツ)_/¯</param>
        /// <param name="emissiveColors">Color to make glow</param>
        /// <returns></returns>
        public static VFXPool CreateMuzzleflash(string name, List<string> spriteNames, int fps, List<IntVector2> spriteSizes, List<tk2dBaseSprite.Anchor> anchors, List<Vector2> manualOffsets, bool orphaned, bool attached, bool persistsOnDeath,
            bool usesZHeight, float zHeight, VFXAlignment alignment, bool destructible, List<float> emissivePowers, List<float> emissiveColorPowers, List<Color> emissiveColors)
        {
            ...
            for (int i = 0; i < spriteNames.Count; i++)
            {
                ...
                float emissiveColorPower = emissiveColorPowers[i];
                ...
                def.material.SetTexture("_MainTexture", def.material.GetTexture("_MainTex"));
                def.material.SetColor("_EmissiveColor", emissiveColor);
                def.material.SetFloat("_EmissiveColorPower", emissiveColorPower);
                def.material.SetFloat("_EmissivePower", emissivePower);


                def.materialInst.shader = mat.shader;
                def.materialInst.SetTexture("_MainTexture", def.materialInst.GetTexture("_MainTex"));
                def.materialInst.SetColor("_EmissiveColor", emissiveColor);
                def.materialInst.SetFloat("_EmissiveColorPower", emissiveColorPower);
                def.materialInst.SetFloat("_EmissivePower", emissivePower);
                frame.spriteCollection = VFXCollection;
                clip.frames = clip.frames.Concat(new tk2dSpriteAnimationFrame[] { frame }).ToArray();
            }

            sprite.renderer.material.shader = mat.shader;
            sprite.renderer.material.SetTexture("_MainTexture", sprite.renderer.material.GetTexture("_MainTex"));
            sprite.renderer.material.SetColor("_EmissiveColor", emissiveColors[0]);
            sprite.renderer.material.SetFloat("_EmissiveColorPower", emissiveColorPowers[0]);
            sprite.renderer.material.SetFloat("_EmissivePower", emissivePowers[0]);
